...
class Runtime:
    ...
    def collect(self, project: str, source_id: str, observations: list, metadata: dict | None = None) -> dict:
        ...
        result = self.store.collect(project_id, source_id, observations, metadata or {})
        return {**result, "project_id": project_id}

    def put_wiki(self, project: str, pages: list, metadata: dict | None = None) -> dict:
    ...
    def schedule(self) -> list[str]:
        ...
        with self.store.transaction() as db:
            for project in db.execute("SELECT id FROM projects").fetchall():
                key = project["id"]
                raw = [r["id"] for r in db.execute("SELECT id FROM observations WHERE project=? AND consumed_by IS NULL ORDER BY id", (key,))]
                active = db.execute("SELECT 1 FROM jobs WHERE project=? AND stage='raw' AND state!='done'", (key,)).fetchone()
                if len(raw) >= self.config.raw_threshold and not active:
                ...
                for skill in targets:
                    ...
                    active = db.execute("SELECT 1 FROM jobs WHERE skill=? AND state!='done'", (skill["id"],)).fetchone()
                    if len(pending) >= self.config.wiki_threshold and not active:
                        created.append(self._enqueue(db, key, "skill", skill["id"], pending))
        return created

    @staticmethod
    def _enqueue(db, project: str, stage: str, skill: str | None, inputs: list[int]) -> str:
        key = uuid.uuid4().hex
    ...
    def _context(self, job: dict) -> dict:
        ...
        if job["stage"] == "raw":
            rows = self.store.rows(f"SELECT id,body FROM observations WHERE id IN ({placeholders}) ORDER BY id", job["inputs"])
            context["observations"] = [{"id": r["id"], **json.loads(r["body"])} for r in rows]
        else:
    ...
    def _apply(self, job: dict) -> dict:
        ...
        with self.store.transaction() as db:
            if job["stage"] == "raw":
                ...
                db.executemany("UPDATE observations SET consumed_by=? WHERE id=? AND consumed_by IS NULL",
    ...
    def drain(self) -> int:
        ...
        while True:
            self.schedule()
            jobs = self.store.rows("SELECT id FROM jobs WHERE state NOT IN ('done','failed') ORDER BY created LIMIT 1")
            if not jobs:
                return count
    ...
    def wake(self) -> dict:
        ...
        return {"worker": "requested", "pid": process.pid}

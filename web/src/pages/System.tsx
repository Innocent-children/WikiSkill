  ...
  return (
    <div className="settings-layout">
      <section className="panel document-panel">
        ...
        <p>
          先选择下面的整理方式，再进入工作台查看原始记录、整理知识文档和生成
          Skill。
        </p>
        <p>
          默认手动选择会话；开启自动模式后持续采集并定时整理。缺少模型设置时仍可查看和导入记录。
        </p>
        <div className="manage-actions">
          <a href={href("manage")}>
            进入知识工作台 <ArrowRight size={14} />
          </a>
        </div>
        <details>
          <summary>可选：连接 Codex MCP</summary>
          <p>
            采集本机轨迹可以独立运行。需要 Codex
            查询或写入知识时，运行下面的命令，将输出的 mcp_command 注册为本机
            stdio MCP，然后重新连接客户端。
          </p>
      ...
      <section className="panel document-panel">
        ...
        {services.data && (
          <p role="status">
            控制面板：
            {services.data.state === "running"
              ? "运行中"
              : services.data.state === "degraded"
                ? "部分后台已停止"
                : "独立页面，后台未连接"}
            {" · "}原文采集：
            {services.data.collector === "running"
              ? "运行中"
              : services.data.collector === "disabled"
                ? "手动模式，未监听"
                : "未运行"}
            {" · "}知识整理：
            {services.data.worker === "running" ? "运行中" : "未运行"}
          </p>
        )}
        <p className="muted small">
          关闭页面或终端后后台继续运行。停止后可再次运行 wikiskill 打开面板。
        </p>

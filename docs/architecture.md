...
批次保存原始记录 ID；先按会话分组，缺少会话 ID 时按来源 identity 分组，再由 `Runtime._raw_batches` 按 `Config.max_turns_per_batch` 划分完整已结束轮次。部分选中轮次补齐其待处理记录，未结束轮次及其后续轮次等待；未归属记录随邻近轮次带入，没有可识别轮次时单独成批。入队后固定记录范围，新增内容留待后续批次。自动批次在排队与执行前检查 [静默条件](../README.md#分析与重试)，已开始的模型请求继续完成。

`Config.session_wait_minutes` 由现有设置接口保存。worker 每轮加载配置，drain 在批次之间检测配置文件变化并重新加载；`Runtime._automatic_session_ready` 将分钟换算为统一截止时间，检查最近入库记录及源文件修改时间。排队与执行前共用此函数，调度频率仍由 `analysis_interval_minutes` 控制。

已有等待、执行中或失败批次的会话暂停再次入队；一次提交可产生多个顺序批次。`Runtime._raw_predecessor` 按批次原记录顺序识别同会话前序未完成批次，`_job_ready` 同时检查轮次结束和自动静默；drain、直接执行和手动入口保持这一顺序。失败只阻塞对应会话。首次执行读取最新 Wiki 并保存上下文；`_apply` 在同一事务中保存 Wiki、来源和消费标记。后批使用前批更新后的 Wiki，无独立汇总阶段。普通重试复用可恢复输入和结果；明确重新生成才按当前轮次数拆分原批输入，保留首批 ID 并为剩余批次创建 ID，`job.rebatched` 记录原输入及所有分批 ID，执行顺序按原记录而非新增批次的创建时间。
